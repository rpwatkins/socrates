...
func Test_ImageBlockMacro(t *testing.T) {
	...
	newIncs := checkMaster(fs, "master.adoc")
...
func Test_InlineImage(t *testing.T) {
	assert := assert.New(t)
	fs := afero.NewMemMapFs()
	// initial a project
	...
	img := fmt.Sprint("here is a cool: image:test.png[]")
	contents := string(file)
	new := fmt.Sprintf("%s\n\n%s", contents, img)
	// write it back to fs
	if err := afero.WriteFile(fs, c, []byte(new), 0644); err != nil {
...
func Test_URL(t *testing.T) {
	...
	url3 := "link:http://www.google.com[]"
	url4 := "link:http://derf.sdewssedsadfftre.com[]"
	url5 := "http://www.google.com[Google]"
	url6 := "http://derf.sdewssedsadfftre.com[Google]"
	url7 := "link:http://www.google.com[Google]"
	url8 := "link:http://derf.sdewssedsadfftre.com[Google]"

	contents := string(file)
	new := fmt.Sprintf("%s\n\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n", contents, url1, url2, url3, url4, url5, url6, url7, url8)
	// write it back to fs
	if err := afero.WriteFile(fs, c, []byte(new), 0644); err != nil {
		fmt.Print(err)
	}
